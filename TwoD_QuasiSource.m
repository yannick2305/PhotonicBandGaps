...
%
% Objective: 
%   This implementation simulates the system under illumination by 
%   alpha-quasiperiodic sources. As a result, it excites complex bands 
%   associated with the quasiperiodicity of the sources.
%
% Input: 
%   - w: A range of frequencies within the bandgap.
%   - alpha: The quasiperiodicity parameter of the sources.
%
% Output: 
%   A vector containing the frequency and the corresponding decay length 
%   for the given alpha-quasiperiodicity.
...
% --- Define the system parameters---
    ...
    % Set quasiperiodicity of the sources 
    alpha = pi;     % The second vector entry is projected away anyway
...
% --- Generate the quasieriodic source ---
    G_a  = zeros(n, n);
    for i = i_min:i_max
        G_a(1,i + i_max+1) = exp(1i * alpha * i);
    end
    G_a = G_a(:);
...
% --- Iterate over the frequencies  ---
    for idx = 1:numIterations
        ...
        % --- Apply quasiperiodic sources ---
        g_a_w = matrix_inverse * G_a;
        values = reshape(g_a_w, n, n);  % Reshape the vector to an n x n matrix
    
        % --- Get the decay via the slope of the surface plot ---
        decay_beta = real(log(values(3, middle))-log(values(1, middle))) / (2);
        combinedResults(idx, :) = [decay_beta, w];
    
        % --- Debugging for adjusting the slope values ---
        %plotMatrixSurface(values)  % Plot the defect mode in a log scalae
        %values = values(:);        % Convert the matrix back to a vector
    end
...
%
% Auxiliary Functions: 
%   These functions are utilized in the computation of the truncated 
%   capacitance matrix.
...
function integral_value = Cm(i, j)
    ...
    Nq          = 15;   % Exponential convergence, use 15.
...
function T = generateToeplitz(i, n)
    ...
    parfor j = 0:n-1 %maybe use parfor
